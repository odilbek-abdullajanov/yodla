...
const Icons = {
  ...
  trash: '<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',
  heart: '<svg class="icon-svg" viewBox="0 0 24 24" fill="currentColor"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>',
  check: '<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>',
  x: '<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>',
  ...
  arrowDown: '<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><polyline points="19 12 12 19 5 12"/></svg>',
  star: '<svg class="icon-svg" viewBox="0 0 24 24" fill="currentColor"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>',
  close: '<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>'
};
...
const SoundEffects = {
  ...
  playCorrect() {
    ...
    this.playTone(659.25, "sine", 0.15, now + 0.08);
  },
  ...
  playWrong() {
    ...
    if (!this.ctx) return;
    const now = this.ctx.currentTime;
    this.playTone(220, "sawtooth", 0.15, now);
    this.playTone(180, "sawtooth", 0.25, now + 0.1);
  },
...
// ================= DATA STORAGE =================
const STORAGE_KEY = 'leitner-words-v2';
...
async function loadData() {
  ...
  const oldWords = !localWords ? await StorageHelper.get('leitner-words-v1') : null;
  ...
  if ((localWords && Array.isArray(localWords) && localWords.length > 0) || (oldWords && oldWords.length > 0)) {
    ...
    const newWordsToAdd = missingDefaults.map((item, idx) => {
      ...
      return {
        ...
        box: 1,
        unit: getUnitForIndex(globalIdx),
        nextReviewDate: new Date().toISOString(),
        nextReview: new Date().toISOString()
      };
    });

    const combinedWords = [...cleanedWords, ...newWordsToAdd];
    ...
    if (newWordsToAdd.length > 0 || cleanedWords.length < sourceWords.length) {
      await saveData();
    }
  } else {
...
function updateLeitnerBoxProgress() {
  ...
  for (let box = 1; box <= 5; box++) {
    ...
    div.setAttribute('data-box', box);
    div.innerHTML = `
      <div class="drawer-level">${box}</div>
      <span class="count">${boxWords.length}</span>
      <div class="due">${dueCount > 0 ? dueCount + ' ta' : ''}</div>
    `;
  ...
  const greeting = due > 0
    ? `Takrorlash uchun ${due} ta so'z tayyor! Qutini bosib boshlang.`
    : `Ajoyib! Tanlangan unitlarda hozircha takrorlash uchun so'zlar yo'q. Erkin mashqlar orqali shug'ullaning!`;
...
function getRandomGreeting() {
  const greetings = [
    "Salom! Bugun yangi so'zlarni yodlashga tayyormisiz?",
    ...
    "Miya mashg'ulotlar uchun tayyor! Bugun qaysi unitdan boshlaymiz?",
      ...
      <div>
        <div class="card-top">
          <div class="card-en-word" data-en="${word.en}">
            <span>${word.en}</span>
            <button class="speak-mini-btn" title="Talaffuz qilish">${Icons.volume}</button>
          </div>
          <button class="btn-delete-word" data-id="${word.id}" title="So'zni o'chirish">${Icons.trash}</button>
        </div>
...
function startSession(mode) {
  ...
  const poolWords = getFilteredPool();
...
function renderCurrentQuestion() {
  ...
  btnFooter.className = "btn btn-primary btn-footer";
...
function checkAnswer() {
  ...
  if (question.type === 1 || question.type === 2 || question.type === 7) {
    ...
    if (question.type === 6 || question.type === 3) {
      isCorrect = normalizeEnglish(userTyped) === normalizeEnglish(correctVal);
      correctExplanation = question.correctAnswer;
    } else {
  ...
  btnFooter.style.display = "block";
  btnFooter.className = isCorrect ? "btn btn-primary btn-footer" : "btn btn-danger btn-footer";
  ...
  if (isCorrect) {
    ...
    footerMessage.innerHTML = `
      <div class="message-icon correct">${Icons.check}</div>
    ...
    if (selectedBtn) {
      ...
      selectedBtn.style.backgroundColor = "var(--primary-soft)";
    }
    ...
    if (selectedBtn) {
      ...
      selectedBtn.style.backgroundColor = "var(--danger-soft)";
    }
    ...
    if (correctBtn) {
      ...
      correctBtn.style.backgroundColor = "var(--primary-soft)";
    }
...
function showSummaryPage(isGameOver) {
  ...
  const iconCircle = document.getElementById("summary-icon-circle");

  if (isGameOver) {
    if (title) title.textContent = "Jonlaringiz tugadi!";
    if (subtitle) subtitle.textContent = "Xafa bo'lmang, xatolar ustida ishlab, yana urinib ko'ring!";
    if (iconCircle) {
      iconCircle.className = "summary-icon-circle wrong";
      iconCircle.innerHTML = Icons.x;
    }
  } else {
    ...
    if (iconCircle) {
      iconCircle.className = "summary-icon-circle success";
      iconCircle.innerHTML = Icons.check;
    }
    triggerConfetti();
  }
